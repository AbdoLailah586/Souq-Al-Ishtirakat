...
export const Navbar: React.FC<NavbarProps> = ({ currentTab, setCurrentTab }) => {
  ...
  const { openTopUpModal, settings, orders } = useStore();
    ...
    <header className="sticky top-0 z-40 bg-bazaar-bg/90 backdrop-blur-md border-b border-bazaar-border shadow-xl">
      ...
      {settings.showBanner && settings.bannerText && (
        <div className="bg-gradient-to-r from-bazaar-purple/30 via-bazaar-gold/20 to-bazaar-teal/30 border-b border-bazaar-border py-1.5 px-4 text-center text-xs sm:text-sm font-medium text-amber-200 flex items-center justify-center gap-2">
          <Sparkles className="w-3.5 h-3.5 text-bazaar-gold animate-spin" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4">
        ...
        >
          <div className="relative w-11 h-11 rounded-2xl bg-gradient-to-br from-bazaar-card to-bazaar-surface border border-bazaar-gold/40 flex items-center justify-center shadow-glow-gold transition-all duration-300 group-hover:scale-105 group-hover:border-bazaar-gold">
            ...
            <span className="absolute -top-1 -right-1 flex h-3 w-3">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-bazaar-gold opacity-75"></span>
              <span className="relative inline-flex rounded-full h-3 w-3 bg-bazaar-gold"></span>
            </span>
          </div>
          <div>
        ...
        {/* Action Controls (Wallet, Role Switcher, Profile) */}
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Role Switcher Pill for Testing */}
          <div className="hidden md:flex items-center bg-bazaar-card/80 border border-bazaar-border rounded-xl p-0.5 text-xs">
            <button
              onClick={() => switchRole('customer')}
              className={`px-2.5 py-1 rounded-lg transition-all ${
                !isAdmin 
                  ? 'bg-bazaar-teal/20 text-bazaar-teal font-bold border border-bazaar-teal/30' 
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded-lg transition-all flex items-center gap-1 ${
                isAdmin 
                  ? 'bg-bazaar-gold/20 text-bazaar-gold font-bold border border-bazaar-gold/30' 
                  : 'text-slate-400 hover:text-white'
              }`}
            >
              <span>إدارة</span>
          ...
          {user && (
            <div className="flex items-center bg-bazaar-card border border-bazaar-border hover:border-bazaar-gold/40 rounded-xl p-1 sm:px-3 sm:py-1.5 transition-all shadow-inner">
              ...
              >
                ...
                <span className="hidden xs:inline">شحن</span>
              </button>
            </div>
          )}
          ...
          <div className="relative">
            ...
            {/* Profile Dropdown */}
            {isProfileMenuOpen && (
              <div className="absolute left-0 mt-2 w-56 rounded-2xl bg-bazaar-card border border-bazaar-border shadow-2xl p-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="p-2 border-b border-bazaar-border/60">
                  ...
                  <span className={`inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full font-bold ${
                    ...
                    {isAdmin ? 'مدير المتجر (Admin)' : 'عميل مميز (VIP)'}
                  </span>
                </div>
                ...
                {/* Switch Role in Mobile / Dropdown */}
                <div className="pt-1 border-t border-bazaar-border/60">
                  <button
                    onClick={() => {
                    ...
                    className="w-full text-right px-3 py-2 rounded-lg text-xs text-bazaar-teal hover:bg-white/5 flex items-center justify-between"
                  >
                    <span>تبديل الوضع إلى ({isAdmin ? 'عميل' : 'إدارة'})</span>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-bazaar-bg/98 border-b border-bazaar-border px-4 pt-2 pb-6 space-y-1">
          ...
          <div className="pt-3 border-t border-bazaar-border flex items-center justify-between">
            <span className="text-xs text-slate-400">وضع المستخدم:</span>
            <div className="flex gap-2">
              <button
                onClick={() => switchRole('customer')}
                className={`px-3 py-1 rounded-lg text-xs font-bold ${
                  !isAdmin ? 'bg-bazaar-teal text-bazaar-bg' : 'bg-bazaar-card text-slate-400'
                }`}
              >
                عميل
              </button>
              <button
                onClick={() => switchRole('admin')}
                className={`px-3 py-1 rounded-lg text-xs font-bold ${
                  isAdmin ? 'bg-bazaar-gold text-bazaar-bg' : 'bg-bazaar-card text-slate-400'
                }`}
              >
                مدير
              </button>
